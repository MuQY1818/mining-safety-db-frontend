...
- 2025-09-02：完全解决聊天历史记录功能-会话列表和消息内容显示全部修复；**影响面**：chatStore.ts三处API调用参数修复(getSessions两处+getMessages一处，统一添加page:1参数，pageSize限制在20以内)，ChatHistoryPanel.tsx字段引用修复(session.messages.length→messageCount)，解决后端200000参数错误问题，会话列表和点击会话查看消息内容功能完全恢复；**回滚**：恢复原有API参数和字段引用；**相关**：chatStore.ts:219-221+427-431+792-794行参数修复，ChatHistoryPanel.tsx:156行，历史记录功能完整恢复
- 2025-09-02：彻底解决聊天历史记录显示问题-Playwright深度调试发现并修复API参数错误；**影响面**：chatStore.ts两处getSessions调用修复参数传递(添加page:1，pageSize:50→20)，解决后端返回200000参数错误导致的"暂无对话记录"问题，ChatHistoryPanel.tsx字段引用修复(session.messages.length→messageCount)，历史记录功能完全恢复显示14个会话；**回滚**：恢复原有参数传递和字段引用；**相关**：chatStore.ts:219-221+792-794行参数修复，ChatHistoryPanel.tsx:156行字段修复，Playwright端到端调试验证
- 2025-09-02：修复聊天历史记录显示问题-会话列表正常显示消息数量；**影响面**：ChatHistoryPanel.tsx第156行修复字段引用从session.messages.length改为session.messageCount，解决后端API正常返回数据但前端显示"暂无对话记录"的问题，历史记录面板现在能正确显示会话数量和消息统计；**回滚**：恢复session.messages.length引用；**相关**：ChatHistoryPanel.tsx:156行字段修复，后端API验证通过
- 2025-09-02：AI聊天消息持久化callback链修复-部分成功但需要验证完整链路；**影响面**：修复siliconflow.ts的onComplete回调类型签名支持async函数，添加完整debug日志追踪callback执行，确认流式API层正确调用onComplete(返回undefined)，但ai.ts中间层callback链路需要进一步验证以确保消息正确保存到后端数据库；**回滚**：恢复原有callback类型签名，移除debug日志；**相关**：siliconflow.ts:21+137-144行callback类型修复，ai.ts:20+28-43行wrapper逻辑，chatStore.ts:266+294-324行消息持久化逻辑
...
- 2025-09-02：NewAIChat.tsx架构重构完成-修复AI消息持久化问题；**影响面**：完整重构NewAIChat.tsx移除直接siliconFlowService调用，统一使用chatStore.sendMessage()确保callback链路正确执行，修复message.type→message.role字段引用，项目编译通过仅轻微eslint警告，AI聊天消息现在通过完整链路(NewAIChat→chatStore→ai.ts→siliconflow.ts)自动保存到后端数据库；**回滚**：从git历史恢复旧版NewAIChat.tsx直接siliconflow调用方式；**相关**：NewAIChat.tsx完整重构，ai.ts:28-54行callback包装器，siliconflow.ts:137-144行onComplete处理，chatStore.ts消息持久化逻辑
- 2025-09-02：AI聊天核心问题三项修复完成-流式响应、内容提取、会话历史；**影响面**：siliconflow.ts修复isStreamEnd函数防止误判结束信号，增强内容提取逻辑支持delta.content/message.content/根级content多种格式，chatStore.ts优化initialize方法加强会话历史加载和本地数据同步，修复TypeScript类型安全问题；**回滚**：恢复原有isStreamEnd逻辑，移除多格式内容提取，恢复原有initialize实现；**相关**：siliconflow.ts:133-275行流式响应修复，chatStore.ts:104-181行会话初始化优化
- 2025-09-02：修复刷新后会话历史不显示问题-页面刷新后正确加载当前会话消息；**影响面**：chatStore.ts的initialize方法新增当前会话消息加载逻辑，调用setCurrentSession为currentSession加载历史消息，确保页面刷新后用户能看到之前的对话记录，验证API数据流和React组件状态管理正常；**回滚**：移除initialize方法中第179-190行的会话消息加载逻辑；**相关**：chatStore.ts:179-197行会话消息加载修复，NewAIChat.tsx状态响应验证
- 2025-09-02：添加完整调试日志系统深度修复会话历史加载-详细追踪API请求响应和数据流；**影响面**：chatStore.ts和chatHistoryService.ts添加完整调试日志系统，追踪用户认证、本地数据加载、API响应、数据转换全流程，NewAIChat.tsx优化初始化时序添加延迟确保authStore完成，增强错误处理提供用户友好提示，创建调试指导文档；**回滚**：移除所有console.log调试语句，恢复原有初始化逻辑，删除调试指导.md；**相关**：chatStore.ts:66-138+158-284行调试日志，chatHistoryService.ts:63-93+127-162行API日志，NewAIChat.tsx:54-62行初始化优化，调试指导.md
- 2025-09-02：修复Zustand持久化导致的初始化跳过问题-移除isInitialized持久化确保刷新重新初始化；**影响面**：移除chatStore的persist配置中isInitialized状态持久化，简化initialize方法的重复检查逻辑，确保每次页面刷新都执行完整的chatStore初始化流程，添加详细状态追踪日志；**回滚**：恢复persist配置中isInitialized持久化，恢复原有重复初始化检查逻辑；**相关**：chatStore.ts:152-167行初始化逻辑简化，908-915行persist配置修复
- 2025-09-02：诊断后端API返回data为null的问题-前端适配处理空数据响应；**影响面**：chatStore.ts增强API响应验证处理data为null情况，chatHistoryService.ts添加详细的后端响应分析和sessionId验证，确定问题根源在后端getMessages接口返回空数据而非前端加载逻辑；**回滚**：移除API null数据处理和详细验证日志；**相关**：chatStore.ts:425-439行API响应处理，chatHistoryService.ts:172-193行后端数据诊断，sessionId验证逻辑
- 2025-09-02：优化session切换性能-实现消息缓存和调试日志优化；影响面：chatStore.ts消息缓存机制+chatHistory.ts调试日志优化；回滚：移除缓存逻辑，恢复JSON.stringify调试输出；相关：perf(chat)性能优化commit 49bb552
