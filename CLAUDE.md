...
- 2025-09-13：重构AI聊天架构-从前端直调改为后端代理SSE流式响应；**影响面**：移除前端SiliconFlow API key配置和直接调用，修改src/services/ai.ts适配后端/api/chat/ai SSE接口，更新chatStore.ts移除前端消息保存逻辑（后端自动保存），禁用siliconflow.ts文件，所有环境配置文件中API key已注释标记为已弃用，实现完整的安全架构升级；**回滚**：恢复siliconflow.ts，重新启用环境变量中的API key配置，恢复ai.ts中的直接调用逻辑；**相关**：src/config/api.ts注释SILICONFLOW_CONFIG，src/services/ai.ts完整重写chatStream方法，src/store/chatStore.ts简化onComplete回调，.env*文件API key配置注释
...
- 2025-09-22：完成代码提交和分支同步操作-主要修改token认证和Docker配置；**影响面**：git add所有修改文件，创建commit包含AI token认证修复、1Panel部署配置、后端SSE流式响应适配，dev分支与main分支已同步(无冲突)，所有修改已进入main分支；**回滚**：git reset --hard HEAD~1恢复提交前状态；**相关**：CLAUDE.md、Dockerfile、src/services/ai.ts、src/components/AIChat/ChatInterface.tsx、docker-compose.1panel.yml
